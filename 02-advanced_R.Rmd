...
$\widehat{\sigma}^{2}=\frac{1}{n}\sum_{i=1}\widehat{\epsilon}^{2}$,
$E\left[x_{i}x_{i}'\right]=\frac{1}{n}X'X$. However, under heteroskedasticity
\[
\sqrt{n}\left(\widehat{\beta}-\beta_{0}\right)\stackrel{d}{\to}N\left(0,E\left[x_{i}x_{i}'\right]^{-1}\mbox{var}\left(x_{i}\epsilon_{i}\right)E\left[x_{i}x_{i}'\right]^{-1}\right)
\]
where $\mbox{var}\left(x_{i}\epsilon_{i}\right)$ can be estimated by 
\[
\underset{\mbox{opt 1}}{\frac{1}{n}\sum_{i=1}^{n}x_{i}x_{i}'\widehat{\epsilon}_{i}^{2}}=\underset{\mbox{opt 2 and 3}}{\frac{1}{n}X'DX}=\underset{\mbox{opt 4}}{\frac{1}{n}\left(X'D^{1/2}\right)\left(D^{1/2}X\right)}
\]
...
\caption{Log into `econsuper`}
\end{figure}

1. Log in `econsuper.econ.cuhk.edu.hk`;
2. Upload `loop_remote.R` it to the server;
...
\begin{figure}
\centering
\includegraphics[width = 15cm]{graph/econ_super_top}
\caption{Running 24 cores on `econsuper`}
\end{figure}

```
# Starting Tuning with Coordinates at: 2020-03-21 22:44:28 
registerDoParallel(24)
...
tc <- Sys.time() - tc
# Time difference of 9.158825 mins

```

* Check the script `loop_remote.R`. 
