...
code the first and the second derivatives of an objective function if it is univariate,
or the $K$-dimensional gradient and  the $K\times K$-dimensional Hessian matrix if
...
In general, integration is more difficult
...
some elements unobservable to the econometric dictate an economic agent's decision.
...
In the original Roy model, a man can work on  one of the two occupations: a farmer or a fisher.
...
In reality, we observe the binary outcome $y=\mathbf{1}\{U_1^{*}> U_2^{*}\}$. If
...
A *binding function* is a mapping from the parameter space of the reduced-form to that
of the structural form. Once the reduced-form parameter is estimated, we recover the
...
is parametric and we can simulate artificial error $(e_{i1}^*, e_{i2}^*)$ conditional on $x_i$. In each simulation experiment,
...
$$
Q(\theta) = \left(\hat{b} - S^{-1} \sum_{s=1}^H \hat{b}(\theta)^{*s} \right)' W \left(\hat{b} - S^{-1} \sum_{s=1}^S \hat{b}(\theta)^{*s}\right)
$$   
where $s$ indexes the simulation, and $W$ is a positive definite weighting matrix.
The indirect inference estimator is $\hat{\theta} = \arg\min_{\theta} Q(\theta).$ That is, we seek the $\theta$
...
of irregular behavior of the objective function. @chernozhukov2003mcmc's *Laplace-type estimator* (LTE) is an alternative to circumvent the challenge in optimization. LTE transforms the value of the criterion function
...
and $\pi(\theta)$ is density of a prior distribution.
The smaller is the value of the objective function, the larger it weighs.

We use MCMC to simulate the distribution of $\theta$.
From a Bayesian's viewpoint, $f_n(\theta)$ is the posterior distribution. However, @chernozhukov2003mcmc use this distribution
for classicial estimation and inference, and they justify the
procedure via frequentist asymptotic theory. Once $f_n(\theta)$
is known, then *asymptotically* the point estimator equals its mean under the quadratic loss function, and equals its median under the absolute-value loss function.
...
It is better to categorize the EM algorithm as a numerical optimization method assisted by simulation. will cite arcidiacono2003finite)
