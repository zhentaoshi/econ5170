...
I decide to call it "simulation".

Simulation has been widely used for (i) checking finite-sample performance of asymptotic theory; (ii) bootstrap, an automated inference procedure, is indeed a simulation exercise;
(iii) generating non-standard distributions; (iv) approximating integrals with no analytic expressions. In this lecture, we will focus on job (i) and (ii), whereas (iii) and (iv)
...
purpose. A piece of script of thousand of lines is not written from the beginning
to the end. It is developed recursively. We cut a big job into small managable tasks.
We write a small piece, test it, and perhaps encapsulate it into a user-defined function.
Small pieces are connnected by the super structure, and finally add comments to facilitate
readability. If a whole script if written by yourself, without comments you will forget
...
(I cannot exactly recall where I learned this solution. Perhaps it was from Liangjun Su when he was at Guanghua School of Management, Peking University. He was my instructor of postgraduate Mathematical Statistics.)

```{r,cache=TRUE}
n = 2 * 10000
Z =  matrix( 2 * ( runif(n)-0.5 ) , ncol = 2 )
# uniform distribution centered at zero, ranging (-1,1)
...
Simulation is one way to evaluate how good is an approximation.

Real data empirical example can also be used for illustration, but artificial data
have a particular advantage. The prevalent paradigm that evaluates an estimator is
to assume that the data comes from a model, and we check how close is the estimate from
the model, often characterized by a set of unknown parameters. In simulation
we have a full control of the data generation process, so we also actually know the
true parameter.
In a real example, we do not really know the true model, so we cannot directly
evaluate the parameter estimation.

(It would be a different story if we are mostly interested in prediction, as we often
do in machine learning. In such cases, we can split the data into two parts, one part
...
stated.
Is the OLS asymptotic theory---consistency and asymptotic normality---valid when $X$ follows a Cauchy distribution?
...
print( Sys.time() - pts0 )
```
When we look at the table, we witness that the distributio of $X$ indeed
does not matter. What matters is the distribution of the error term.
The firm column is the when the error is normal, and we use the exactly distribution theory to find the critical value (according to the $t$ distribution.)
The second column still use the normal as the error distribution, but change the
critical value to be from the normal distribution, which is based on the asymptotic approximation. When sample size is small, obvious size distortion is
observed; but the deviation is mitigated when the sample size increases.
When the distribution is Cauchy, the so called "exact distribution" is not longer exact. If we attempt to use the asymptotic normal approximation, we find the asymptotic conclusion is broken. The test size does not converge to the
normial 5% as sample size increases.
