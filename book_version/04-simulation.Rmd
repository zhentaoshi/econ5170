...
Monte Carlo, the European casino city state, is another unfortunate presence.
However, naming it Macau simulation or Hong Kong Jockey Club simulation does not make me feel any better.
I decide to call it "simulation", just like we do not have to eat sashimi with rice.

Simulation has been widely used for (i) checking the finite-sample performance of asymptotic theory;
(ii) generating non-standard distributions; (iii) approximate integrals with no analytic expressions;
(iv) bootstrap.
...
Zu Chongzhi (429–500 AD), an ancient Chinese mathematician, calculated $\pi$ being between 3.1415926 and 3.1415927.
He was using a deterministic approximation algorithm, and held the world record for 900 years.
 Now imagine that we present to Zu Chongzhi, with full respect and admiration,
a modern PC. How can he achieve a more accurate approximation? Of course, we suppose that he would not google it.

Standing on the shoulder of law of large numbers, $\pi$ can be approximated is by stochastic approximation.

(I cannot exactly recall where I learned this example. Perhaps it was from Liangjun Su when he was an
  assistant professor at Guanghua School of Management, Peking University.)
...
the test size of the OLS estimator.
