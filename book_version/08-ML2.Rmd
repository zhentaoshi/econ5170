...
However, insufficient theoretical understanding is shed light on these methods due to
...
@gu2018empirical showcases a horse race of a myriad of methods,
...
that fall short of scientific standard. Extreme caution must be exercised when
...
Regression tree is an alternative method. It  recursively partitions the
...
from the data DGP, the  covariate chosen to be split and the splitting point can be
...
variance of the regression tree. Bagging grows a regression tree for
...
$$
It is an example of the *ensemble learning*.
...
Bagging and random forest almost always use equal weight on each generated tree
 for the ensemble.
Tree boosting takes a distinctive scheme to determine the ensemble weights.
...
For example, the script `Beijing_housing_gbm.R` achieves 99.5+% out-of-sample $R^2$ (in comparison with OLS's 88%) in the Beijing housing data.
This script implements boosting via the package `gbm`, which stands for "Gradient Boosting Machine."
...
a_l^{(k)} & = & g^{(k)} ( z_l^{(k)}), \nonumber
\end{eqnarray}
where $x_j = a_j^{(0)}$ is the input.
The above formulation shows that  $z_l^{(k)}$ usually takes a linear form,
while the *activation function* $g(\cdot)$ can be an identify function or a simple nonlinear function.
Popular choices of the activation function are sigmoid ($1/(1+\exp(-x))$) and rectified linear unit (ReLu, $z\cdot 1\{x\geq 0\}$), etc.


\begin{figure}
\centering
\includegraphics[width = 5cm]{Colored_neural_network}
\caption{A Single Layer Feedforward Neural Network}
\end{figure}
...
scientists, Google's `tensorflow` is perhaps the most popular backend of
...
SGD uses a small batch of the sample
