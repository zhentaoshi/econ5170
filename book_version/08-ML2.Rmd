...
A widely used nonparametric method is the kernel regression.
Regression tree is an alternative method that recursively partition the
the space of the regressors.

* Algorithm: each time split a covariate to aggressively reduce the SSR. The
tuning parameter is the number of splits.

Given a dataset $d$, the fitted regression tree $\hat{r}(d)$ completely determined
by the data.
...
from the data DGP, the splitting covariate and the splitting point can be
different and they heavily influences the path of the partition.

Bootstrap averaging, or *bagging* for short, fits a regression tree for
each bootstrap sample, and then do a simple average. Let $d^{\star b}$
be the $b$-th bootstrap sample of the original data $d$. Then
$$
\hat{r}_{\mathrm{bagging}} = B^{-1} \sum_{b=1}^B \hat{r}( d^{\star b} ).
$$
The simple average over bootstrap samples reduces the variability of the regression trees.
This is an example of the *ensemble learning*.
...
*Random forest* shakes up the regressors by randomly sampling $m$ out of the total $p$ covarites before *each split of a tree* in a bagging process. The tuning parameters in random forest
is the tree depth and $m$. Due to the "de-correlation" in sampling the regressors, it in
general performs better than bagging in prediction.
...
Despite the simplicity of the algorithm, the theory of random forest is not proved
until a few years ago[@scornet2015consistency].
...
Bagging and random forest almost always uses equal weight on each generated tree.
Tree boosting takes a different scheme to determine the ensemble weights.
It does not resample the original data.

1. Use the original data $d^0=(x_i,y_i)$ to grow a shallow tree $\hat{r}^{0}(d^0)$. "Shallow" means only a few splits. Save the prediction $f^0_i = \alpha \cdot \hat{r}^0 (d^0, x_i)$ where
...
3. Repeat Step 2 until $m > M$, where $M$ is a tuning parameter of the number of total iterations.

In this boosting algorithm there are three tuning parameters: the tree depth,  $\alpha$, and $M$.
The algorithm can be sensitive too all three tuning parameters.
When a model is tuned well, it often performs remarkably well.
...
tree in each iteration is the deepest descent direction, while the shrinkage makes sure that
it does not proceed too aggressively.

In R, boosting can implemented by the package `gbm`, which stands for "Gradient Boosting Machine."

* Example: Beijing housing price. Achieves 99.5% out of sample $R^2$.

* @shi2016econometric employs the greedy algorithm behind boosting to select information moments in GMM.
* Phillips and Shi (2018, to be released) uses $L_2$-boosting as a boosted version of the Hodrick-Prescott filter.
...
* Efron and Hastie: Chapter 8 and 17
