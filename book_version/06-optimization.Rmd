...
$|\theta_{t+1} -\theta_{t}|/|\theta_{t}| < \epsilon$ (relative criterion), where $\epsilon$ is a small positive number chosen as the tolerance level.
...
whether a root to $s(\theta)$ is associated to a minimizer or a maximizer, and we compare the values of the mimima to decide a
global minimum.
...
**Quasi-Newton method.** The most well-known quasi-Newton algorithm is  [BFGS](http://en.wikipedia.org/wiki/Broyden%E2%80%93Fletcher%E2%80%93Goldfarb%E2%80%93Shanno_algorithm).
...
For example, we can access [`NLopt`](http://ab-initio.mit.edu/wiki/index.php/NLopt_Installation) through the packages [`nloptr`](http://cran.r-project.org/web/packages/nloptr/index.html).
...
To invoke BFGS in NLOPT, we must provide the gradient, 
as in the function `poisson.log.grad()` below. 
Here we compare it with the numerical gradient to make sure 
