...
% Functions
    \draw [color = red, thick] (2,7) .. controls (6,2) and (10,7) .. (10,7) node[left]{MSE}; % MSE
    \draw (1,1)     node[above]{bias$^2$}   parabola (11,7); % bias^2
...
Suppose $y_i = x_i' \beta_0 + e_i$, where $e_i$ is orthogonal to $x_i$ and $\mathrm{var}[e_i] = \sigma^2$. Then
$$
...
In reality, we have a sample $(y_i, x_i)$ of $n$ observations, and we estiamted $\beta$ with the OLS estimator $\hat{\beta} = (X'X)^{-1}X'y$. 
...
< \sigma^2
$$
Asymptotically, if $p/n \to 0$, the two risks converge. Otherwise if $p/n \to c$, then the expected SSR is strictly 
