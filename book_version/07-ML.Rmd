...
is only about $X$. Instances of the former are various regressions; those of the latter are density estimation,
principle component analysis, and clustering. These examples are all familiar econometric problems.
...
% Nodes
  ...
  \node[ellipse, draw=blue,fill=black!0, right= of d, text width=3.5cm, align=center] 
    (e) {Best Parameter \\ (Model)};
  \node[ellipse, draw, left = 10 mm of d, loosely dashed] 
    (f)
    [label = above left: \textit{\textcolor{black!40}{Many Sets of}}, 
    label = below: \textcolor{black!40}{$Model$}]
    {Tuning Parameter};
...
% Caption    
  \node[below = of d] {
        \begin{tabular}{l}
            ...
            $\bullet$ A data driven approach for future selection
        \end{tabular}};
\end{tikzpicture}
)
...
In this lecture, we set aside the ongoing philosophical debate. Instead,  we study
...
% Nodes
    ...
    \node [color=black!40](c) at (0,5) {Population};
    \node (d) at (5,4) {Estimation};
    \node [color=black!40] (e) at (0,3) {Sample};
    \node (f) at (5,2) {Inference};
    \node (g) at (5,0) {Interpretation};
    \node [color=black!40](h) at (0,1) {Economics};
...
% Lines
    \draw[color=black!40,very thick] (0,2) -- (2,2);
...
They use some "off-the-shelf" learning methods, which economists often view as "black-box".
They are flexible to allow a large number of parameters.
The models are measured by their performance in prediction.
In order to avoid overfitting it is often necessary to tune at least a few tuning parameters.
...
in a data rich environment they often split the data into three parts. A training dataset
...
It helps decide a set of optimal tuning parameters. Ideally, the testing sample should be
kept by a third party out of the touch of the modeler. The testing sample is the final
judge of the superiority of the fitted model.

The R package `caret` provides a framework for many macine learning methods.
The function [`createDataPartition`](https://topepo.github.io/caret/data-splitting.html) 
splits the data for both cross sectional data and time seies.
