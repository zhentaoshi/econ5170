...
A `plyr` loop. It saves house keeping chores, and easier to parallelize. We encapsulate
the chunk in the loop as a new function `capture`, and run the loop via `__ply`.
`__ply` is a family of functions. Here `ldply` means the input is a list and the output is a data frame.
...
Parallel computing will be our next topic. It is quite easy to implement parallel execution with `plyr`.

## Parallel Computing

The packages `foreach` and `doParallel` are useful for parallel computing.
Below is the basic structure. `registerDoParallel(number)` prepares other CPU cores
to accept jobs. Next, when calling `__ply`, we explicitly specify `.parallel` to be TRUE.
If `myfunction` also depends on other variables in the session and other packages,
we need to export them to other cores via the argument `.paropts` (parameter options).
...
In theory, if we have two CPUs running simultaneously, we can cut the time to a half of that on a single CPU.
Is it true?
...
Surprisingly, the above block of parallel computing indeed takes more time.
