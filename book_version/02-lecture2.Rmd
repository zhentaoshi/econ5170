...
In theory, if we have two CPUs running simultaneously, we cut the time to a half of that on a single CPU. Is that what happening in practice?
...
Investing money from our own pocket for a powerful laptop to conduct heavy-lifting computational work
will surely be a regret. (i) We do not run these long jobs every day, it is more cost efficient
to share a workhorse. (ii) We cannot keep our laptop always on when we take it
around. The right solution is remote computing on a server.

There is no fundamental difference between local and remote computing.
We prepare the data and code, open a shell for communication, run the code, and collect the results.
What might be unfamiliar is that ususally we have to deal with a command-line-based operation system.
Such command line tools is the norm of life two or three decades ago, but today we mostly
work in a graphic operating system like the Windows or OSX.
...
Here are a few commands for basic operating in a Unix/Linux operating system.
...
Our CUHK department runs two servers, one for students and the other for professors.
The one for professors is a 16-core server. I have opened an account for you.
